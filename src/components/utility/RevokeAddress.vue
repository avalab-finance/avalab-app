<template>
  <div class="relative flex flex-col w-full h-full px-4 py-5 justify-start rounded-3xl bg-opaqueDark-light dark:bg-opaqueDark-dark">
    <h1 class="w-full dark:text-avalabGreen mb-4">Revoke Address</h1>
    <div class="w-full flex flex-wrap gap-x-3">
      <input type="text" v-model="address" class="flex ss:w-7/12 md:w-5/12 h-8 rounded-xl items-center ring-black st-5 ring-opacity-10 focus:ring-avalabGreen ring-1 focus:outline-none bg-slightGray dark:bg-slightDark pl-2" placeholder="Enter your wallet address">
      <div class="ss:w-7/12 md:w-full">
        <v-select placeholder="Choose a Token" :options="options" label="title" v-model="selectToken"  :on-change="checkSelectValue" class="ss:7/12 md:w-5/12 my-4 rounded-xl ring-black st-5 ring-opacity-10 focus:ring-avalabGreen ring-1 focus:outline-none bg-slightGray dark:bg-slightDark">
      ...
      <input v-if="showField" type="text" v-model="inputToken" class="flex md:w-5/12 ss:w-full h-8 rounded-xl items-center ring-black st-5 ring-opacity-10 focus:ring-avalabGreen ring-1 focus:outline-none bg-slightGray dark:bg-slightDark pl-4" placeholder="Enter a custom token">
      <span class="w-full">
