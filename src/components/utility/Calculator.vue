<template>
  <div id="calculator" class="max-w-screen-xl mx-auto flex flex-1 flex-col items-center avalab-layout xl:px-0 px-3 text-gray-500 pb-16" style="margin-top: 98px">
      <div  class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 w-full">
        <div v-for="(pair, idx) in this.Pools" :key="idx" class="flex flex-none flex-col h-a p-3 st5 st5-all group bg-gradient-to-l dark:from-slightDark from-slightGray to-transparent dark:hover:bg-slightDark hover:bg-slightGray border-l border-avalabGreen rounded-3xl">
...
export default {
  ...
  computed: {
    ...mapGetters("addressConstants", ["AVALabCHEF", "UNIROUTERV2", "hMULTICALL", "hRPC"]),
  },
  methods: {
