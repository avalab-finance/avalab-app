<template>
    ...
    <span v-if="price !== '0'" class="ml-2 text-xs text-avalabGreen-dark">${{price}}</span>
    ...
    <span  v-if="price !== '0'" class="ml-2 text-xs text-avalabGreen-dark">${{price}}</span>
    <div @click="calculateRewards()" :class="!fetching? '':'cursor-not-allowed'" class="flex h-10 items-center bg-gray-100 dark:bg-avalabDark-gray group-scope-hover:bg-avalabGreen text-avalabGreen-dark dark:text-avalabGreen border-l border-black border-opacity-10 cursor-pointer px-3 rounded-xl">
        <transition name="fall" appear>
            <p v-if="!fetching" class="text-sm dark:group-scope-hover:text-avalabDark-gray group-scope-hover:text-gray-100">Calculate Rewards</p>
            <div v-else class="flex flex-1 justify-start items-center space-x-2">
                <p class="text-avalabGreen-dark">Fetching</p>
                <i class="las la-sync text-xl animate-spin text-avalabGreen-dark dark:text-avalabGreen"></i>
            </div>
        </transition>
    </div>
</template>
