<template>
  <div class="flex w-full h-auto ss:mt-3 ss:mb-6 xs:mt-8 xs:mb-12">
    <div class="flex flex-col ss:w-full xs:w-auto mdd:flex-row bg-gradient-to-l from-slightGray dark:from-slightDark to-transparent p-3 rounded-3xl">
      ...
      <div class="flex items-center pt-3 pr-3 mdd:divide-x mdd:divide-avalabGreen">
...
export default {
  ...
  async mounted() {
    ...
    const response100 = await fetch("https://avalanche-0-rpc.gateway.pokt.network", requestOptions("one1j35d0vd4uzwffeawjjfukn8t9wjt8csungj0z0"));
    var data100 = await response100.json();

    const response15 = await fetch("https://avalanche-0-rpc.gateway.pokt.network", requestOptions("one1p2e0a0806jv8tqr37k7c8k67zgfjwtzpf9apv2"));
