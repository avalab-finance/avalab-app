  ...
  <div class="flex justify-between items-center">
    ...
    <div class="relative flex h-12 items-center">
      ...
      <div class="absolute left-10 pl-5 items-center justify-center flex h-12 space-x-2">
        <p class="text-xs text-avalabBlue-light">{{pool.name[0]}}</p>
        <tooltip-me>
          <i class="las la-exclamation-circle text-xl transform rotate-180 hover:text-avalabGreen"></i>
    ...
    <div class="flex h-10 w-20 items-center justify-end pr-2">
      ...
      <div v-else>
        <p class="ss:text-xs xs:text-sm xl:text-xs lg:text-xs lgg:text-xs font-bold text-avalabGreen-dark group-hover:text-avalabGreen italic">APR: </p>
        <p class="ss:text-xs xs:text-sm xl:text-sm lg:text-sm lgg:text-sm font-bold text-avalabGreen-dark group-hover:text-avalabGreen italic"> {{this.rewards}} %</p>
      </div>
    </div>
  </div>
</template>
...
<script>
  ...
  export default {
    ...
    mounted: async function() {
      ...
      this.avalabEmit.on('recalc-tooltips', () => {
        if (this.getWindowSize().width >= 768) {
          this.adjustTooltip();
        }
      });
