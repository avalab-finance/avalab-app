  ...
  <div class="flex flex-1 items-center">
    ...
    <div class="flex flex-col space-y-1 w-1/3 min-w-0">
      ...
      <div class="relative flex space-x-2 items-center h-10">
        <i @click="addTokenToMetamask(this.getToken()['token1'])" class="flex las la-plus text-sm p-1 text-avalabGreen absolute top-0 left-0 items-center justify-center bg-slightGray dark:bg-slightDark rounded-full cursor-pointer shadow hover:bg-avalabGreen dark:hover:bg-avalabGreen hover:text-slightGray dark:hover:text-slightDark"></i>
        <img :src="this.getToken()['token1'].imgSrc" class="h-8 w-8 rounded-full flex items-center justify-center" alt="">
        <div class="flex flex-1 items-center min-w-0">
          <p class="text-sm text-el text-avalabGreen-dark">{{this.getToken()['token1'].Symbol}}</p>
        </div>
      </div>
      <!-- Network name -->
      ...
      <div class="flex h-10">
        <Input0 :errors="errors" :rounded="'rounded-xl'" :placeholder="'Amount...'" :errorTop="'pt-10'">
          <div class="flex flex-1 st5-all items-center justify-end group-scope z-30 absolute right-0">
            <div @click="setMax()" class="flex h-10 items-center bg-gray-100 dark:bg-avalabDark-gray group-scope-hover:bg-avalabGreen text-avalabGreen-dark dark:text-avalabGreen border-l border-black border-opacity-10 cursor-pointer px-3 rounded-xl">
              <p class="text-sm dark:group-scope-hover:text-avalabDark-gray group-scope-hover:text-gray-100">MAX</p>
            </div>
          </div>
        </Input0>
      </div>
      <div class="flex pl-2 space-x-2 h-4">
        <p class="text-xs flex-none dark:text-avalabGreen-dark">Balance :</p>
