<template>
  <div class="relative">
    <div @click="toggleCustomRate" :class="checkCustom ? 'border-l-2 border-avalabGreen' : 'hover:bg-gray-50 dark:hover:bg-slightDark'" class="flex items-center justify-center cursor-pointer w-8 h-6 relative hover:bg-gray-50 dark:hover:bg-slightDark dark:bg-avalabDark-gray bg-gray-100 rounded-lg focus:outline-none">
    ...
    >
      <div class="absolute z-50 w-48 mt-2 right-0" v-if="isOpen">
        <div class="relative flex flex-col space-y-2 py-2 border-l-2 border-avalabGreen shadow-lg rounded-lg p-1 px-2 bg-gray-100 dark:bg-slightDark">
          <div class="flex items-center space-x-2">
            <p class="dark:text-gray-300">Custom Rate</p>
            <tooltip-me>
              <i class="las la-exclamation-circle text-sm transform rotate-180 dark:text-gray-300 dark:hover:text-avalabGreen hover:text-avalabGreen cursor-pointer"></i>
          ...
          <InputWithValidation @keyup.enter="toggleCustomRate" :input="amount" :errors="errors" @catchInput="inputAmount" :rounded="'rounded-lg'" :placeholder="'%'" :errorTop="'pt-8'">
            <p class="flex items-center justify-center text-xs z-30 right-0 absolute bg-gray-100 dark:bg-avalabDark-gray rounded-xl px-3 h-8">%</p>
          </InputWithValidation>
        </div>
      </div>
    </transition>
