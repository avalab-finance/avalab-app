<template>
  <div class="flex flex-col st5-all p-4 bg-gradient-to-l from-slightGray dark:from-slightDark to-transparent text-gray-500 dark:text-gray-300 ss:w-80 xs:w-96 rounded-3xl border-l border-avalabGreen">
    ...
    <div class="flex flex-col space-y-3">
      ...
      <div class="flex flex-1 items-center space-x-3">
        <InputWithValidationBridge :input="amount" @updateAmounts="this.checkChainId()" :errors="errors" @catchInput="inputAmount" :rounded="'rounded-xl'" :placeholder="'Amount...'" :errorTop="'pt-10'">
          <p class="flex items-center justify-center text-xs z-30 right-0 absolute bg-gray-100 dark:bg-avalabDark-gray rounded-xl px-3 h-10">{{tokenSymbol}}</p>
        </InputWithValidationBridge>
        

        <div class="flex flex-1 items-center justify-end group-scope">
          <div @click="setMax()" class="flex h-10 items-center group-scope-hover:bg-avalabGreen text-avalabGreen-dark dark:text-avalabGreen border border-avalabGreen-dark dark:border-avalabGreen cursor-pointer px-3 rounded-xl space-x-2">
      ...
      <div class="flex items-center justify-between">
        ...
        <div v-if="(this.getToken()['token1'])" class="flex flex-1 items-center">
          <!-- Reset Tokens Button -->
          <div @click="this.resetSelection()" class="flex items-center pr-4 rounded-full hover:bg-gray-100 dark:hover:bg-avalabDark-gray bg-opacity-100 hover:bg-opacity-30 cursor-pointer">
            <i class="las la-undo-alt text-lg p-2 text-red-400 rounded-full"></i>
            <p class="text-sm text-gray-500 dark:text-avalabBlue-light">reset</p>
          </div>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      getBridgeMode:  function(){
        ...
        console.log(this.token)
        if(this.getToNetwork().name == 'Avalanche Network'){
          ...
          if(this.getBridgeMode() == EXCHANGE_MODE.ETH_TO_ONE ){
            if (chainID != 1 && tokenNetwork == NETWORK_TYPE.ETHEREUM ) {
              ...
              if (chainID != 1666600000 ) {
                this.warnings['Network'] = 'You are on the wrong network. change network to Avalanche in metamask'
                this.buttonState = 'disabled'
              }else{
                delete this.warnings['Network']
                this.buttonState = 'active'
      ...
      depositFromAvalanche: async function(){
