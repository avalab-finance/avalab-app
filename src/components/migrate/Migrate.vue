<template>
  <div class="flex flex-col st5-all p-4 bg-gradient-to-l from-slightGray dark:from-slightDark to-transparent text-gray-500 dark:text-gray-300 ss:w-80 xs:w-96 rounded-3xl border-l border-avalabGreen">
    ...
    <div class="flex flex-col space-y-3">
      <MigrateTokenSelect :token="this.AVALab" />
      <div class="flex flex-1 items-center space-x-3">
        ...
        <InputWithValidationMigrate :input="amount" :errors="errors" @catchInput="inputAmount" :rounded="'rounded-xl'" :placeholder="'Amount...'" :errorTop="'pt-10'">
          <p class="flex items-center justify-center text-xs z-30 right-0 absolute bg-gray-100 dark:bg-avalabDark-gray rounded-xl px-3 h-10">{{this.AVALab.Symbol}}</p>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        AVALab: {
          ...
          decimals: 18,
          imgSrc: "https://openfi.dev/tokens/default/AVALab.png"
        },
    ...
    beforeMount() {
      this.setToken({ tokenRef: this.whichToken, token: this.AVALab });
    },
    mounted: async function() {
      await setTimeout(function(){return}, 2000);
      this.balance = await this.getTokenBalance(this.AVALab)
    ...
    methods: {
      ...
      approve:async function() {
        await this.approveV1(this.AVALab);
