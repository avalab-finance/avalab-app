<template>
  ...
  <transition tag="div" name="approve-btn" class="inline-block absolute">
    <div @click="executeRemoveLiquidity" v-if="this.getBtnState({remove: 'remove'})" class="flex w-28 group">
      ...
      <div class="grab-attention cursor-pointer st5">
        <div class="flex flex-1 items-center justify-center">
          <p class="text-sm text-avalabGreen-dark dark:text-avalabGreen group-hover:text-gray-50 dark:group-hover:text-avalabDark-gray">Remove</p>
        </div>
        <i class="las la-minus text-xl text-avalabGreen-dark dark:text-avalabGreen group-hover:text-gray-50 dark:group-hover:text-avalabDark-gray"></i>
      </div>
    </div>
  </transition>
  ...
  <transition tag="div" name="approve-btn" class="inline-block absolute">
    <div v-if="this.getBtnState({remove: 'removing'})" class="flex w-28 group">
      ...
      <div class="grab-attention cursor-wait st5">
        <div class="flex flex-1 items-center justify-center">
          <p class="text-sm text-avalabGreen-dark dark:text-avalabGreen group-hover:text-gray-50 dark:group-hover:text-avalabDark-gray">Removing</p>
        </div>
  ...
  <!-- Removed -->
  <transition tag="div" name="approve-btn" class="inline-block absolute">
    <div v-if="this.getBtnState({remove: 'removed'})" class="flex w-28 st5 justify-between items-center border border-avalabGreen glow-avalabGreen-light-md space-x-1 p-2 pl-3 rounded-full dark:bg-avalabDark-gray bg-gray-100 cursor-default">
      <div class="flex flex-1 items-center justify-center">
        <p class="text-sm text-avalabGreen">Removed</p>
      </div>
