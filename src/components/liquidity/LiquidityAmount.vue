  ...
  <div class="flex flex-1 flex-col">
    <div class="flex flex-1 items-center space-x-3 mb-3 z-30">
      <InputWithValidationLiquidity @click="setInputClicked('1')" :input="amount0" :errors="error0" @catchInput="inputAmount0" :rounded="'rounded-xl'" :placeholder="'Amount...'" :errorTop="'pt-10'">
        <div class="flex items-center text-xs z-30 right-0 absolute bg-gray-100 dark:bg-avalabDark-gray rounded-xl h-10">
          <p class="px-3">{{token0.Symbol}}</p>

          <div v-if="pair" class="flex flex-1 st5-all items-center justify-end group-scope">
            <div @click="setMax0()" class="flex h-10 items-center bg-gray-100 dark:bg-avalabDark-gray group-scope-hover:bg-avalabGreen text-avalabGreen-dark dark:text-avalabGreen border-l border-black border-opacity-10 cursor-pointer px-3 rounded-xl">
    ...
    <div class="flex flex-1 items-center space-x-3 z-20">
      <InputWithValidationLiquidity @click="setInputClicked('2')" :input="amount1" :errors="error1" @catchInput="inputAmount1" :rounded="'rounded-xl'" :placeholder="'Amount...'" :errorTop="'pt-10'">
        <div class="flex items-center text-xs z-30 right-0 absolute bg-gray-100 dark:bg-avalabDark-gray rounded-xl h-10">
          ...
          <div v-if="pair" class="flex flex-1 st5-all items-center justify-end group-scope">
            <div @click="setMax1()" class="flex h-10 items-center bg-gray-100 dark:bg-avalabDark-gray group-scope-hover:bg-avalabGreen text-avalabGreen-dark dark:text-avalabGreen border-l border-black border-opacity-10 cursor-pointer px-3 rounded-xl">
              <p class="text-sm dark:group-scope-hover:text-avalabDark-gray group-scope-hover:text-gray-100">MAX</p>
            </div>
          </div>
        </div>
