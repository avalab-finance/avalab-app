<template >
  <div id="walletState" class="pl-1 lg:pl-3">
    ...
    <div v-if="getUserSignedIn === false"  @click="connect()" class="flex st5 lg:w-44 items-center text-gray-500 space-x-1 lg:pr-2 p-1 rounded-full dark:bg-gray-700 dark:hover:bg-gray-600 bg-gray-200 hover:bg-gray-100 cursor-pointer ring-1 ring-black ring-opacity-5">
      <div class="flex flex-none relative h-8 w-8 items-center justify-center st5 bg-gray-200 dark:bg-avalabDark-gray rounded-full">
    ...
    <div v-if="getUserSignedIn" class="flex st5 lg:w-44 items-center space-x-1 lg:pr-2 p-1 rounded-full bg-opacity-20 hover:bg-opacity-50 cursor-pointer border-avalabGreen-dark border glow-avalabGreen-light-md hover:bg-white dark:hover:bg-gray-700">
      <div class="flex flex-none relative h-8 w-8 items-center justify-center st5 bg-avalabGreen rounded-full hover:bg-yellow-400">
        <transition name="fade-in-and-rotate" appear>
          <i class="absolute las la-wallet text-xl st5 text-white dark:text-avalabDark-gray"></i>
        </transition>
      </div>
      <div @click="disconnect()" class="flex w-full items-center p-1 text-sm justify-center dark:text-avalabGreen">
