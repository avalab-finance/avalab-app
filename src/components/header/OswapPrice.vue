<template>
  ...
  <tooltip-me>
    <div class="flex rounded-lg focus:outline-none focus:ring-1 focus:ring-black space-x-2 p-2 px-1 md:px-3 items-center">
      <img alt="AVALab" src="@/assets/avalab_asset.png" class="ss:h-5 xs:h-6">
      <p class="ss:hidden xs:block text-xs dark:text-avalabGreen">${{avalabPrice}}</p>
    </div>

    <tooltip-me-content :options="tooltip" class="flex text-xs flex-col divide-y-2 divide-avalabGreen w-72 p-0.5 rounded-lg shadow-xl">
      <div class="flex flex-col p-3 bg-gray-100 dark:bg-slightDark rounded-md text-gray-500 dark:text-gray-300">
        <div class="ss:flex ss:mb-3 xs:hidden ss:space-x-2 ss:items-center">
          <img alt="AVALab" src="@/assets/avalab_asset.png" class="h-5">
          <p>OpenX Price: ${{avalabPrice}}</p>
        </div>
      ...
      <div class="flex flex-col p-3 bg-gray-100 dark:bg-slightDark rounded-md text-gray-500 dark:text-gray-300">
        ...
        <div class="flex space-x-2 mb-3 items-center">
          <i class="las la-globe-europe text-xl text-avalabBlue-light"></i>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      this.avalabPrice = await this.getOpenXPrice();
      let tempBalances = await this.getBurnAndTotalSupply();
      this.marketCap = commify((tempBalances.circSupply * this.avalabPrice).toFixed(2));
