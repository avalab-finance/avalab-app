<template>
  <div id="pairs" class="max-w-screen-xl mx-auto flex flex-1 flex-col items-center avalab-layout xl:px-0 px-3 text-gray-500 pb-16" style="margin-top: 98px">
    <transition name="fall" appear>
      <div  v-if="loaded" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 w-full">
        <!-- icons table -->
        <div v-for="(pair, idx) in this.Pools" :key="idx" class="flex flex-none flex-col p-3 st5 st5-all group bg-gradient-to-l dark:from-slightDark from-slightGray to-transparent dark:hover:bg-slightDark hover:bg-slightGray border-l border-avalabGreen rounded-3xl">
          <div class="flex flex-1 space-x-2 items-center">
            <div class="relative w-20">
              <i @click="addTokenToMetamask({oneZeroxAddress:pair.pairAddress})" class="z-10 flex las la-plus text-sm p-1 text-avalabGreen absolute -top-8 items-center justify-center bg-slightGray dark:bg-slightDark rounded-full cursor-pointer shadow hover:bg-avalabGreen dark:hover:bg-avalabGreen hover:text-slightGray dark:hover:text-slightDark"></i>
            ...
            <div class="flex flex-1 flex-col md:space-y-2 lg:space-y-2 space-y-4">
              <p class="text-base text-avalabBlue-light">
                {{ pair.pair }}
              </p>
              <div class="flex space-x-2 h-5 items-center mt-2">
                <i class="las la-tint dark:text-avalabGreen"></i>
              ...
              <div class="flex space-x-2 h-5 items-center">
                <i class="las la-hand-holding-usd dark:text-avalabGreen"></i>
      ...
      <div v-else class="flex h-full items-center mt-16">
        <svg class="animate-spin h-8 w-8 text-avalabGreen" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
...
export default {
  ...
  components: { Pair },
    computed: {
    ...mapGetters("addressConstants", ["AVALabCHEF", "UNIROUTERV2", "hMULTICALL"]),
  },
  methods: {
