<template>
  <div id="farm" class="max-w-screen-xl mx-auto flex flex-1 flex-col avalab-layout xl:px-0 px-3 text-gray-500 pb-16">
    <div v-if="!validatorData" class="flex flex-1 items-center justify-center w-full h-full">
      <transition name="fade-in" appear>
        <svg class="animate-spin h-7 w-7 text-avalabGreen" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      ...
      <div v-if="validator" class="flex flex-col w-full space-y-3 mt-8 text-gray-600 dark:text-gray-300">
        <div class="flex flex-col w-full space-y-4">
            ...
            <div class="flex w-full col-span-2">
              <div class="flex flex-col">
                ...
                <div class="flex space-x-2 items-center mb-6">
                  <i class="las la-sitemap text-2xl dark:text-avalabGreen"></i>
          ...
          <div class="grid grid-cols-8 gap-3">
            <div v-for="(delegator, index) in validator.delegators" :key="index" class="flex flex-col rounded-xl space-y-1 st5 rounded-2xl bg-gradient-to-l dark:from-slightDark from-slightGray to-transparent dark:hover:bg-slightDark hover:bg-slightGray border-l border-avalabGreen py-3 px-4 shadow">
              <div class="flex flex-col space-y-1">
                <p class="text-xs text-avalabBlue-light">Address:</p>
                <p class="text-xs text-el">{{delegator['delegator-address']}}</p>
                <p class="text-xs text-avalabBlue-light">Amount:</p>
                <p class="text-xs text-el">{{prettify((delegator.amount/10**18).toFixed(2))}}</p>
                <p class="text-xs text-avalabBlue-light">Weight:</p>
...
<script>
  ...
  export default {
          ...
          chartOptions: {
            ...
            tooltip: {
              ...
              custom: ({ series, seriesIndex, dataPointIndex, w }) => {
                return (
                  '<div class="flex p-3 rounded-md bg-gray-100 dark:bg-slightDark text-xs text-gray-500 dark:text-gray-200 border-l border-avalabGreen">' +
    ...
    async mounted() {
      ...
      const response = await fetch("https://avalanche-0-rpc.gateway.pokt.network", requestOptions);
