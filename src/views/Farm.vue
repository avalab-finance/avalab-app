<template>
  <div id="farm" class="max-w-screen-xl mx-auto flex flex-1 flex-col items-center justify-center avalab-layout xl:px-0 px-3 text-gray-500">
...
export default {
  ...
  computed: {
    ...mapGetters("addressConstants", ["AVALabMAKER", "AVALabCHEF", "hMULTICALL", "hRPC"]),
  },
  methods: {
    ...
    */
    ...
    getTotalPending: async function () {
      ...
      for (var n in this.SoloPools) {
        ...
        CALL.push({
          target: MASTERCHEF,
          call: ["pendingAVALab(uint256,address)(uint256)", parseInt(this.SoloPools[n].pid), userAddress],
      ...
      for (var n in this.Pools) {
        ...
        CALL.push({
          target: MASTERCHEF,
          call: ["pendingAVALab(uint256,address)(uint256)", parseInt(this.Pools[n].pid), userAddress],
          returns: [["PENDING_OF_" + n, (val) => val]],
        });
      }
    ...
    initMulticall: async function (pools) {
      //const OPENMAKER = this.AVALabMAKER();
      const MULTICALL = this.hMULTICALL(this.getChainID());
      const RPC = this.hRPC(this.getChainID());
    ...
    generateCalls: function (pools) {
      ...
      for (var n in pools) {
        ...
        CALL.push({
          target: MASTERCHEF,
          call: ["pendingAVALab(uint256,address)(uint256)", parseInt(pools[n].pid), userAddress],
          returns: [["PENDING_OF_" + n, (val) => val]],
        });
