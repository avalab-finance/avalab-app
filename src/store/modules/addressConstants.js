export default {
  ...
  state: {
    ...
    AVALabChef: {
      ...
      1:"",
      56:"",
      1666600000: "https://avalanche-0-rpc.gateway.pokt.network",//"https://avalanche-0.gateway.pokt.network/v1/lb/618a63ac7335c800360b4ce3",
  ...
  mutations: {},
  getters:{
  	AVALabTOKEN: (state) => (id) => {
  		return state.AVALabToken[id];
  	},
    ...
    eUSDC: (state) => (id) => {
  	...
  	AVALabMAKER: (state) => (id) => {
		return state.AVALabMaker[id];
  	},
  	AVALabCHEF: (state) => (id) => {
		return state.AVALabChef[id];
  	},
  	UNIROUTERV2: (state) => (id) => {
  		return state.UniswapV2Router02[id];
  	},
    OPENSWAPBRIDGE: (state) => (id) => {
      return state.AVALabBridge[id];
    },
  	UNIFACTORY: (state) => (id) => {
  		return state.UniswapV2Factory[id];
  	},
  	hMULTICALL: (state) => (id) => {
  		return state.avalancheMulticall[id];
  	},
  	hRPC:(state) => (id) => {
  		return state.avalancheRPC[id];
  	}
  },
  actions: {},
  modules: {}
