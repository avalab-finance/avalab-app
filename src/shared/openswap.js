import MasterChef from "avalab-v2-core/build/contracts/MasterChef.json";
import IUniswapV2Router02 from "avalab-v2-core/build/contracts/IUniswapV2Router02.json";
import SushiMaker from "avalab-v2-core/build/contracts/SushiMaker.json";
import IERC20 from "avalab-v2-core/build/contracts/IERC20.json";
import AVALabBridge from "avalab-v2-core/build/contracts/AVALabBridge.json";
import AVALabToken from "avalab-v2-core/build/contracts/AVALabToken.json";
import DelegatorContract from "@/components/farm/Validators/delegateContract.json";
import { ethers } from "ethers";

import { Avalanche } from '@avalanche-js/core';
import { toBech32 } from '@avalanche-js/crypto'

import { ChainType, numberToHex, Unit } from '@avalanche-js/utils'
import oneWallet from './OneWallet.js'


const hmy = new Avalanche(
  // let's assume we deploy smart contract to this end-point URL
  'https://avalanche-0-rpc.gateway.pokt.network',
  {
...
const { Route, Price, Token, Fetcher, ChainId, Trade, TokenAmount, TradeType, Percent} = require("avalab-v2-sdk");
...
export default {
  created: function () {},
  computed: {
    ...mapGetters('addressConstants', ['AVALabMAKER', 'AVALabCHEF','WONE' , 'UNIROUTERV2', 'OPENSWAPBRIDGE','AVALabTOKEN', 'bBUSD', 'eBUSD', 'eUSDC', 'bUSDC', 'lockedAddress']),
  ...
  methods: {
    ...
    getProvider: function(call){
      ...
      1666600000: {
        rpcURL: 'https://avalanche-0-rpc.gateway.pokt.network',
        provider: new ethers.providers.JsonRpcProvider('https://avalanche-0-rpc.gateway.pokt.network', {chainId: 1666600000, name: "Avalanche Mainnet"}),
        name: "mainnet avalanche"
      },
      1666700000: {
        rpcURL: 'https://api.s0.b.hmny.io',
        provider: new ethers.providers.JsonRpcProvider('https://api.s0.b.hmny.io', {chainId: 1666700000, name: "Avalanche Testnet"}),
        name: "testnet"
      },
      1337:{
    ...
    getOpenXPrice: async function () {
        ...
        const Avalab = new Token(
          chainID,
          this.AVALabTOKEN(chainID),
          18
        );
        ...
        const pair = await Fetcher.fetchPairData(Avalab, Busd).catch(error => {
          console.log(error);
          this.error = 1;
    ...
    getAllRewards: async function () {
      ...
      if (address != "0x0000000000000000000000000000000000000011") {
        ...
        const masterChef = this.AVALabCHEF(this.getChainID());
        const contract = new ethers.Contract(masterChef, abi, provider);

        for (n in Pools) {
          const pending = await contract
            .pendingAVALab(parseInt(Pools[n].pid), address).catch(error => {
              console.log(error);
              this.error = 1;
    ...
    getSingleRewards: async function(){
      ...
      const contract = new ethers.Contract(masterChef, abi, provider);
      const i = 11;
      const pending = await contract.pendingAVALab(i, address).catch(error => {
        console.log(error);
        this.error = 1;
        this.errormessage = "Error getting reward amount.";
      });;
    ...
    collectAll: async function(){
        ...
        let abi = [
          ...
          if(this.getWalletType() == 'metamask'){
            ...
            const contract = new ethers.Contract(this.AVALabCHEF(this.getChainID()), abi, signer);
            ...
            let explorer = 'https://explorer.avalanche.one/#/tx/'
              toastMe('info', {
          ...
          if(this.getWalletType() == 'oneWallet'){
            ...
            const unattachedContract = hmy.contracts.createContract(abi,this.AVALabCHEF(this.getChainID()));
    ...
    collectAVALAB: async function(pool){
      let isDefaultWallet = this.checkSignedIn()
      if (isDefaultWallet){
        toastMe('error', {
          title: 'ERROR',
    ...
    burnAll: async function(){
      ...
      const provider = new ethers.providers.Web3Provider(window.ethereum);
      const signer = provider.getSigner();
      const contract = new ethers.Contract(this.AVALabMAKER(this.getChainID()), abi, signer);
    ...
    burnPool: async function(pool){
      ...
      if(this.getWalletType() == 'metamask'){
         const provider = new ethers.providers.Web3Provider(window.ethereum);
          const signer = provider.getSigner();
          let makerAddr = this.AVALabMAKER(this.getChainID())
      ...
      if(this.getWalletType() == 'oneWallet'){
        let options = { gasPrice: "0x3B9ACA00" };
        const unattachedContract = hmy.contracts.createContract(abi, this.AVALabMAKER(this.getChainID()));
        let wallet = new oneWallet()
        await wallet.signin()
        let contract = wallet.attachToContract(unattachedContract)
        //const gas = await contract.methods.withdraw(pid, '0').estimateGas(options).catch();
    ...
    getBestRoute: async function(parsedAmount, token0, token1) {
      ...
      BUSDC] = await Promise.all([
        ...
        new Token(
                this.getChainID(),
                this.AVALabTOKEN(this.getChainID()),
                18
                ),
        new Token(
                this.getChainID(),
    ...
    getAvalabPerBlock: async function(){
      const provider = this.getProvider()
      const abi = MasterChef.abi;
      const contract = new ethers.Contract(this.AVALabCHEF(this.getChainID()), abi, provider);

      const avalabPerBlock = await contract.AVALabPerBlock();
      ...
      let rewardsPerWeek = avalabPerBlock.div(allocPoints).mul(blocksPerWeek)
    ...
    getRewardValue: async function(pool, poolWeight) {
      ...
      const price = await this.getAvalabPrice()
      
      const aWeekly = await this.getAvalabPerBlock()
    ...
    setRatioValidator: async function(amount){
      ...
      const tx = await contract.setRatioPercentageAVALab(100 - amount).catch(err => {
          var message;
          if(!err.data?.message){
            message = err.message
          }else{
      ...
      if(this.getWalletType() == 'oneWallet'){
        ...
        var tx1 = await contract.methods.setRatioPercentageAVALab((100 - amount).toString()).send(options)
        if(tx1.transaction.txStatus == 'CONFIRMED'){
          this.setBtnState({add: 'added'})
          let transaction = tx1.transaction.id
          this.setBtnState({remove: 'removed'})
    ...
    unDelegateValidator: async function(amountIn, valAddr){
      ...
      const explorer = 'https://explorer.avalanche.one/#/tx/'
      let wallet = new oneWallet()
        await wallet.signin()
    ...
    getBurnAndTotalSupply: async function() {
      ...
      const AVALabToken = this.AVALabTOKEN(this.getChainID());
      const burnAddress = "0xdEad000000000000000000000000000000000000";

      const abi = AVALabToken.abi
      
      const provider = this.getProvider()
      const contract = new ethers.Contract(AVALabToken, abi, provider);
